...
export default function ClientChatRoute() {
  const { userId: userIdParam, sessionId: sessionIdParam } = useLocalSearchParams<{
    userId: string;
    sessionId?: string;
  }>();
  ...
  const { thread, messages, loading } = useTherapistThreadByUser(userId);
  const threadId = thread?.id ?? null;
  const { sendMessage, loading: sending } = useTherapistSendThreadMessage(threadId);

  const clientName = thread?.user?.fullName ?? `Client #${userId ?? ''}`;
